...
////////////////////////////////////////////////////////////////////////////////
NSString * kxmovieErrorDomainVer2 = @"ru.kolyvan.kxmovie";
...
static NSError * kxmovieError (NSInteger code, id info)
{
    ...
    return [NSError errorWithDomain:kxmovieErrorDomainVer2
                               code:code
                           userInfo:userInfo];
}

static NSString * errorMessage (kxMovieErrorVer2 errorCode)
{
    switch (errorCode) {
...
@interface KxMovieFrameVer2()
@property (readwrite, nonatomic) CGFloat position;
@property (readwrite, nonatomic) CGFloat duration;
@end

@implementation KxMovieFrameVer2
@end

@interface KxAudioFrameVer2()
@property (readwrite, nonatomic, strong) NSData *samples;
@end

@implementation KxAudioFrameVer2
- (KxMovieFrameTypeVer2) type { return KxMovieFrameTypeAudio; }
@end

@interface KxVideoFrameVer2()
@property (readwrite, nonatomic) NSUInteger width;
@property (readwrite, nonatomic) NSUInteger height;
@end

@implementation KxVideoFrameVer2
- (KxMovieFrameTypeVer2) type { return KxMovieFrameTypeVideo; }
@end

@interface KxVideoFrameRGBVer2 ()
@property (readwrite, nonatomic) NSUInteger linesize;
@property (readwrite, nonatomic, strong) NSData *rgb;
@end

@implementation KxVideoFrameRGBVer2
- (KxVideoFrameFormatVer2) format { return KxVideoFrameFormatRGB; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@implementation KxVideoFrameYUVVer2
- (KxVideoFrameFormatVer2) format { return KxVideoFrameFormatYUV; }
@end

@interface KxArtworkFrameVer2()
@property (readwrite, nonatomic, strong) NSData *picture;
@end

@implementation KxArtworkFrameVer2
- (KxMovieFrameTypeVer2) type { return KxMovieFrameTypeArtwork; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@interface KxSubtitleFrameVer2()
@property (readwrite, nonatomic, strong) NSString *text;
@end

@implementation KxSubtitleFrameVer2
- (KxMovieFrameTypeVer2) type { return KxMovieFrameTypeSubtitle; }
@end

////////////////////////////////////////////////////////////////////////////////

@interface KxMovieDecoderVer2 () {
    ...
    NSDictionary        *_info;
    KxVideoFrameFormatVer2  _videoFrameFormat;
    NSUInteger          _artworkStream;
    NSInteger           _subtitleASSEvents;
}
@end

@implementation KxMovieDecoderVer2
...
- (void) setSelectedAudioStream:(NSInteger)selectedAudioStream
{
    ...
    kxMovieErrorVer2 errCode = [self openAudioStream: audioStream];
...
- (void) setSelectedSubtitleStream:(NSInteger)selected
{
    ...
    if (selected == -1) {
        ...
        kxMovieErrorVer2 errCode = [self openSubtitleStream:subtitleStream];
        if (kxMovieErrorNone != errCode) {
            NSLog(@"%@", errorMessage(errCode));
        }
    }
...
{
    KxMovieDecoderVer2 *mp = [[KxMovieDecoderVer2 alloc] init];
    if (mp) {
            ...
            error: (NSError **) perror
{
    ...
    kxMovieErrorVer2 errCode = [self openInput: path];
    
    if (errCode == kxMovieErrorNone) {
        
        kxMovieErrorVer2 videoErr = [self openVideoStream];
        kxMovieErrorVer2 audioErr = [self openAudioStream];
...
- (kxMovieErrorVer2) openInput: (NSString *) path
{
...
- (kxMovieErrorVer2) openVideoStream
{
    kxMovieErrorVer2 errCode = kxMovieErrorStreamNotFound;
    _videoStream = -1;
    _artworkStream = -1;
...
- (kxMovieErrorVer2) openVideoStream: (NSInteger) videoStream
{    
...
- (kxMovieErrorVer2) openAudioStream
{
    kxMovieErrorVer2 errCode = kxMovieErrorStreamNotFound;
    _audioStream = -1;
...
- (kxMovieErrorVer2) openAudioStream: (NSInteger) audioStream
{   
...
- (kxMovieErrorVer2) openSubtitleStream: (NSInteger) subtitleStream
{
    ...
    if (codecCtx->subtitle_header_size) {
        ...
        if (s.length) {
            ...
            NSArray *fields = [KxMovieSubtitleASSParserVer2 parseEvents:s];
...
- (KxVideoFrameVer2 *) handleVideoFrame
{
    ...
    if (_videoFrameFormat == KxVideoFrameFormatYUV) {
        ...
        KxVideoFrameYUVVer2 * yuvFrame = [[KxVideoFrameYUVVer2 alloc] init];
        ...
        KxVideoFrameRGBVer2 *rgbFrame = [[KxVideoFrameRGBVer2 alloc] init];
...
- (KxAudioFrameVer2 *) handleAudioFrame
{
    ...
    KxAudioFrameVer2 *frame = [[KxAudioFrameVer2 alloc] init];
...
- (KxSubtitleFrameVer2 *) handleSubtitle: (AVSubtitle *)pSubtitle
{
    NSMutableString *ms = [NSMutableString string];
        ...
        if (rect) {
            ...
            if (rect->text) { // rect->type == SUBTITLE_TEXT
                ...
                if (s.length) {
                    ...
                    NSArray *fields = [KxMovieSubtitleASSParserVer2 parseDialogue:s numFields:_subtitleASSEvents];
                    if (fields.count && [fields.lastObject length]) {
                        
                        s = [KxMovieSubtitleASSParserVer2 removeCommandsFromEventText: fields.lastObject];
                        if (s.length) [ms appendString:s];
                    }                    
                }
            }
    ...
    KxSubtitleFrameVer2 *frame = [[KxSubtitleFrameVer2 alloc] init];
    frame.text = [ms copy];   
        ...
        if (packet.stream_index ==_videoStream) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    KxVideoFrameVer2 *frame = [self handleVideoFrame];
                    if (frame) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    KxAudioFrameVer2 * frame = [self handleAudioFrame];
                    if (frame) {
            ...
            if (packet.size) {
                ...
                KxArtworkFrameVer2 *frame = [[KxArtworkFrameVer2 alloc] init];
            ...
            while (pktSize > 0) {
                ...
                if (gotsubtitle) {
                    ...
                    KxSubtitleFrameVer2 *frame = [self handleSubtitle: &subtitle];
                    if (frame) {
                        [result addObject:frame];
                    }
...
static int interrupt_callback(void *ctx)
{
    if (!ctx)
        return 0;
    __unsafe_unretained KxMovieDecoderVer2 *p = (__bridge KxMovieDecoderVer2 *)ctx;
...
@implementation KxMovieSubtitleASSParserVer2
